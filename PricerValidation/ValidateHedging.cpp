...
void PnL(VanillaCall* call, MonteCarlo* mc_, PnlMat*  path, int n_time_steps, int H, PnlVect *portfolio_values, PnlVect* option_prices, double& error, double sigma) {
	...
	double prix = 0;
	pnl_vect_resize_from_scalar(portfolio_values, H + 1, 0);
	pnl_vect_resize_from_scalar(option_prices, H + 1, 0);
	PnlMat *past = pnl_mat_create(1, 1);;
	double delta_prev = 0;
	int path_index = 0;
	int hedging_index;
	...
	double prev_spots = pnl_mat_get(path, M - M / H, 0);
	...
	pnl_mat_set_col(comparaison, option_prices, 1);
...
void histogram_errors(int M, BlackScholesModel* model, PnlRng* rng, VanillaCall* call, MonteCarlo* mc, int n_time_steps, double sigma_) {
	...
	model->simul_market(simulated_path, call->T_, M, rng);
	...
	double error = 0;
	PnL(call, mc, simulated_path, n_time_steps, H, portfolio_values, option_values, error, sigma_);
...
void validate_hedging() {
	...
	for (int scenario = 0; scenario < 1; scenario++) {
		...
		PnL(call, mc, simulated_path, n_time_steps, H, portfolio_values, option_values, error, sigma_);
		std::cout << "error at scenario " << scenario + 1 << "  " << error << std::endl;
	}
}
