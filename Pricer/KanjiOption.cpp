...
double KanjiOption::payoffKanji(const PnlMat *path, double rUSD, double rHKD) {
	...
	for (int i = 1; i <= nbTimeSteps_; i++) {
		for (int d = 0; d < 5; d++)
		{
				if (d == 0)
					...
					perf += ((pnl_mat_get(path, i, d) * (pnl_expm1(rUSD * i * timestep) + 1) / pnl_mat_get(path, 0, d - 1)) / (pnl_mat_get(path, 0, d) * (pnl_expm1(rUSD * 0 * timestep) + 1) / pnl_mat_get(path, 0, d - 1))) - 1;
				else if (d == 4)
					perf += ((pnl_mat_get(path, i, d) * (pnl_expm1(rHKD * i * timestep) + 1) / pnl_mat_get(path, i, d - 1)) / (pnl_mat_get(path, 0, d) * (pnl_expm1(rHKD * 0 * timestep) + 1) / pnl_mat_get(path, 0, d - 1))) - 1;
