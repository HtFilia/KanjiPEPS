...
KanjiBlackScholes::KanjiBlackScholes(PnlVect * r, PnlMat * corrMarket, PnlVect * sigma, PnlVect * spot, PnlVect * trend)
{
	...
	pnl_vect_clone(vols, sigma);
	pnl_vect_set(vols, 2, sqrt(pow(pnl_vect_get(sigma_, 2), 2) + pow(pnl_vect_get(sigma_, 1), 2) + 2 * pnl_vect_get(sigma_, 2) * pnl_vect_get(sigma_, 1) * pnl_mat_get(corrMarket_, 1, 2)));
	pnl_vect_set(vols, 4, sqrt(pow(pnl_vect_get(sigma_, 4), 2) + pow(pnl_vect_get(sigma_, 3), 2) + 2 * pnl_vect_get(sigma_, 4) * pnl_vect_get(sigma_, 3) * pnl_mat_get(corrMarket_, 3, 4)));
...
void KanjiBlackScholes::asset(PnlMat * path, double T, int nbTimeSteps, PnlRng * rng)
{
	...
	for (int d = 0; d < size_; d++) {
		...
		for (int i = 1; i <= nbTimeSteps; i++) {
			...
			expo = pnl_expm1((pnl_vect_get(r_, 0) - pnl_pow_i(GET(sigma_, d), 2) / 2)  * timeSpan +
